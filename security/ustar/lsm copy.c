...
static inline int difc_set_task_label(struct task_struct *tsk, label_t label, int operation_type, int label_type, void __user *bulk_label)
{
	int return_val;
...
static int difc_tasks_labels_allowed(struct task_struct *s_tsk,struct task_struct *d_tsk)
{
	...
	if (unlabeled_source_tsk && unlabeled_dest_tsk)
		{	
			difc_lsm_debug(" both tasks are not labeld! so how we eneded up here? domains should be in client mode for both\n");
			return -1;
		}
