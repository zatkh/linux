...
BOOT=/home/zt233/Desktop/boot
FS_MODEULE=/home/zt233/Desktop/rootfs
LINUX_BOOT_DIR=$PWD/arch/arm/boot
LINUX_MODULES_DIR=$PWD/mnt/ext4/lib/modules/4.19.73-v7+/
...
cp -R $LINUX_BOOT_DIR/dts/*.dtb $BOOT/
#sudo cp -R $LINUX_BOOT_DIR/dts/overlays/*.dtb* $FS/boot/overlays/
cp -R $LINUX_MODULES_DIR $FS_MODEULE/
cp $LINUX_BOOT_DIR/zImage $BOOT/kernely.img
cp -R  $LINUX_BOOT_DIR/dts/*.dtb $BOOT/
cp -R $LINUX_BOOT_DIR/dts/overlays/*.dtb* $BOOT/overlays/
